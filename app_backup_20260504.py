...
def get_flights():
    ...
    try:
        r = requests.get(
            "https://api.airplanes.live/v2/point/36.5/127.5/500",
    ...
    try:
        r2 = requests.get(
            ...
            "?lamin=30&lomin=120&lamax=42&lomax=135",
            headers={"User-Agent": "SkyWatch/3.0 (Korea Air Traffic Monitor)"},
            timeout=15
        )
        if r2.status_code == 200:
            for s in (r2.json().get("states") or []):
                ...
                seen[icao] = [
                    ...
                    float(alt_m),
                    on_ground,
                    float(speed_ms),           # m/s (airplanes.live와 단위 동일)
                    float(heading),
                    "",                        # desc 없음
                    s[2] or "",
                ]
...
def search_flight_info(callsign):
    params = {"serviceKey": DATA_GO_KR_KEY, "pageNo": 1, "numOfRows": 10,
              "_type": "json", "flightId": callsign}
    try:
        r = requests.get("http://apis.data.go.kr/1613000/DmstcFlightNvgInfoService/getFlightOpratInfoList",
...
function drawRoute(route) {{
    ...
    if(cur) {{
        L.polyline([dep, cur], {{
            color:'#90a4ae', weight:2.5, dashArray:'6,5', opacity:0.7
        }}).addTo(map);
    }}
    // 남은 경로 (파란 점선)
    L.polyline([cur || dep, arr], {{
        color:'#4fc3f7', weight:2.5, dashArray:'8,6', opacity:0.85
    }}).addTo(map);

    // 출발 마커
    L.marker(dep, {{icon: L.divIcon({{
        className:'',
        html:'<div style="background:#1565c0;color:#fff;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,0.3);">🛫</div>',
        iconAnchor:[14,14]
    }})}}).bindTooltip('<b>' + (route.dep_name||'출발') + '</b>', {{permanent:false}}).addTo(map);

    // 도착 마커
    L.marker(arr, {{icon: L.divIcon({{
        className:'',
        html:'<div style="background:#e65100;color:#fff;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,0.3);">🛬</div>',
        iconAnchor:[14,14]
    }})}}).bindTooltip('<b>' + (route.arr_name||'도착') + '</b>', {{permanent:false}}).addTo(map);

    // 경로 전체 보이게 줌
    var bounds = cur ? [dep, cur, arr] : [dep, arr];
...
airportsData.forEach(function(a) {{
    var icon = L.divIcon({{
        className: '',
        html: '<div class="airport-label">🏢 ' + a.iata + '</div>',
        iconAnchor: [20, 10]
    }});
...
function showInfoPanel(f) {{
    var altFt = Math.round((f.altitude||0) * 3.28084);
    var spd = f.speed || 0;
    var statusIcon = f.on_ground ? '🛬 지상 대기' : '✈️ 비행 중';
    var altColor = f.on_ground ? '#9e9e9e' : (f.altitude > 8000 ? '#4fc3f7' : (f.altitude > 3000 ? '#81c784' : '#ffb74d'));
    infoPanel.innerHTML = [
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">',
        '<div style="font-size:16px;font-weight:700;color:#4fc3f7;letter-spacing:1px;">✈ ' + (f.callsign||'미확인') + '</div>',
        '<div style="font-size:11px;color:#7ab3d4;">' + statusIcon + '</div>',
        '<div style="cursor:pointer;color:#7ab3d4;font-size:18px;line-height:1;" onclick="closeInfoPanel()">×</div>',
        '</div>',
        (f.desc ? '<div style="font-size:11px;color:#b0c4d8;margin-bottom:8px;padding:4px 8px;background:rgba(79,195,247,0.08);border-radius:6px;">' + f.desc + '</div>' : ''),
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px;">',
        '<div style="text-align:center;background:rgba(79,195,247,0.08);border-radius:8px;padding:8px;">',
        '<div style="font-size:15px;font-weight:700;color:' + altColor + ';">' + Math.round(f.altitude||0) + 'm</div>',
        '<div style="font-size:9px;color:#7ab3d4;">고도 / ' + altFt + 'ft</div></div>',
        '<div style="text-align:center;background:rgba(79,195,247,0.08);border-radius:8px;padding:8px;">',
        '<div style="font-size:15px;font-weight:700;color:#81c784;">' + spd + '<span style="font-size:10px;">km/h</span></div>',
        '<div style="font-size:9px;color:#7ab3d4;">속도</div></div>',
        '<div style="text-align:center;background:rgba(79,195,247,0.08);border-radius:8px;padding:8px;">',
        '<div style="font-size:15px;font-weight:700;color:#ce93d8;">' + Math.round(f.heading||0) + '°</div>',
        '<div style="font-size:9px;color:#7ab3d4;">방향</div></div>',
        '</div>',
        '<div style="font-size:11px;color:#7ab3d4;border-top:1px solid #1e4976;padding-top:8px;">',
        '🌏 ' + (f.country||'미확인') + '&nbsp;&nbsp;|&nbsp;&nbsp;📍 ' + (f.lat||0).toFixed(3) + ', ' + (f.lon||0).toFixed(3),
        '</div>',
        '<div style="margin-top:10px;text-align:center;font-size:11px;background:rgba(21,101,192,0.3);border:1px solid #1e4976;border-radius:6px;padding:6px;color:#b0c4d8;">',
        '💡 아래 <b style="color:#4fc3f7">편명 검색창</b>에 <b style="color:#fff">' + (f.callsign||'') + '</b> 입력 후 검색하세요',
        '</div>'
    ].join('');
    infoPanel.style.display = 'block';
}}

// selectFlight 함수에 패널 표시 연동
var _origSelectFlight = selectFlight;
selectFlight = function(callsign) {{
    _origSelectFlight(callsign);
    // 해당 편명 데이터 찾아서 패널에 표시
    var fd = flightsData.find(function(f) {{ return f.callsign === callsign; }});
    if(fd) showInfoPanel(fd);
}};
</script>
</body>
</html>
